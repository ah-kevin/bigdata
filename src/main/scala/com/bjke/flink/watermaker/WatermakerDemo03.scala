...
object WatermakerDemo03 {
  def main(args: Array[String]): Unit = {
    ...
    val orderDs: DataStream[Order] = env.addSource(new SourceFunction[Order] {
      ...
      override def run(ctx: SourceFunction.SourceContext[Order]): Unit = {
        ...
        while (flag) {
          ...
          // 随机模拟延迟
          val eventTime: Long = System.currentTimeMillis() - random.nextInt(20) * 1000
    ...
    // 准备一个outputTag用来存放迟到严重的数据
    val seriousLateOutPutTag: OutputTag[Order] = new OutputTag[Order]("seriousLate")

    val result1: DataStream[Order] = orderDsWithWM.keyBy(_.userId)
      .window(TumblingEventTimeWindows.of(Time.seconds(5)))
      .allowedLateness(Time.seconds(3))
      .sideOutputLateData(seriousLateOutPutTag)
      .sum("money")
    val result2: DataStream[Order] = result1.getSideOutput(seriousLateOutPutTag)

    result1.print("正常的/迟到不严重的数据")
